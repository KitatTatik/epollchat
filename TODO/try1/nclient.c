...
int main(int argc, const char *argv[]) {
    ...
    low = newwin(maxy/4, maxx - maxx/5, maxy * 3/4, 0);
...
//    box(bottom,'|',' ');
    ...
    wsetscrreg(low, maxy * 3/4, maxy - 2);
...
/* not yet
                    while ((strstr(ptr->msg_itself,"/ENDLOG") == NULL)) {
                        ptr->comm = 3;
                        mvwprintw(low, linelow, 1, enter);
                        memset (rbuffer, 0, bsize);
                        usleep(100);
                        bytes_read = read (events[i].data.fd, rbuffer, bsize);
                        rbuffer[bytes_read] = '\0';
                      ...
                      werase(low); 
            not yet*/
                      mvwprintw(low, linelow, 1, "NOT YET");                   
                      box(low,'|','=');
                      wrefresh(low);
                      wrefresh(bottom);
