...
#define CTRLD   4
#define MAX_LEN 10
#define RIGHT_LOGIN "Tigger"
#define RIGHT_PASSWORD "Rabbit"
...
char* get_password(WINDOW * win, char * password, int max_len)
{
    int i = 0;
    int ch;
    while (((ch = wgetch(win)) != 10) && (i < max_len-1)) {
        ...
        password[i++] = ch;
        wechochar(win, '*'); // wechochar(win, ch) for overt text
    }
    password[i] = 0;
    wechochar(win, '\n');
    return(password);
...
//    mousemask(ALL_MOUSE_EVENTS, NULL);
    ...
    //    box(my_menu_win, 0, 0);
	attron(COLOR_PAIR(2));
	mvprintw(LINES - 2, 0, "Use Arrow Keys to navigate, Enter to make choice, F1 to Exit");
    ...
    keypad(wnd0, TRUE);
    wprintw(wnd0, "LOGIN\n");
    get_password (wnd0, login, MAX_LEN);
    if (strcmp(login, RIGHT_LOGIN) == 0) {
        wprintw(wnd0, "WELCOME");
    } else {
        wprintw(wnd0, "USER NOT FOUND");
    }
    wrefresh(wnd0);
    ...
    if (strcmp(password, RIGHT_PASSWORD) == 0) {
        wprintw(wnd, "ACCESS OK");
    } else {
        wprintw(wnd, "WRONG PASSWORD");
    }
    wrefresh(wnd);
    delwin(wnd);
...
/*     mouse_end:
    ...
    mvprintw(LINES - 5, 0, "your choice made");
*/
    wrefresh(my_menu_win);
    unpost_menu(my_menu);
    free_menu(my_menu);
...
//    printf("DEBUG 0001 in_msg_create, %d\n", mptr->len);
...
int main(int argc, const char *argv[]) {
    ...
    int lineright = 1; // Line position on online users list
    ...
    bottom = newwin(maxy/2, maxx - maxx/5, maxy /2, 0);
    ...
    if (ptr->comm == 6) {
        ...
        mvwprintw(right, lineright, 2, enter);
        wrefresh(right);
    ...
    while (running) {
        ...
        for (i = 0; i < num_ready; i++) {
            if (0 != events[i].data.fd) {
                ...
                out_msg_create (ptr, login, enter, 1);
                format_msg(ptr);
                strcpy(enter, ptr->message_str);
                strcpy(sbuffer, enter);
                send (sock, sbuffer, strlen(sbuffer) + 1, 0);
